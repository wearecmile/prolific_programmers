...
//        List<FirebaseNotification> firebaseNotificationsIos = new ArrayList<>();
        List<FirebaseNotification> firebaseNotifications = new ArrayList<>();
        for (Meeting meeting : meetingList) {
            ...
            for (User user : userGroupList) {
                ...
                Optional<UserFcmToken> userFcmToken = userFcmTokenService.findByUserId(user.getUserId());
                if (userFcmToken.isEmpty())
                    continue;
                FirebaseNotification firebaseNotification = FirebaseNotification.builder()
                        .fcmToken(userFcmToken.get().getUftFcmToken())
                        .title("Meeting Scheduled")
                        .content(role + StringConstants.TEXT_SPACE + name + "Scheduled On" + date)
                        .build();
                firebaseNotifications.add(firebaseNotification);
                Notifications notification = notificationFactory.getNotification(userFcmToken.get().getUftDeviceType());
...
//        Notifications notificationsIos = notificationFactories.getNotification("ios");
//        notificationsIos.sendNotificationToAllFcmToken(firebaseNotificationsIos);
//        Notifications notificationsWeb = notificationFactories.getNotification("web");
