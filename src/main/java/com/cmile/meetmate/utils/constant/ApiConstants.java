package com.cmile.meetmate.utils.constant;

public class ApiConstants {

    public static final String REQUEST_MAPPING_KEY_ID = "/{id}";
    public static final String REQUEST_MAPPING_KEY_ATTENDANCE = "/attendance";
    public static final String REQUEST_MAPPING_KEY_SAVE_ALL_ATTENDANCE = "/save-all";
    public static final String REQUEST_MAPPING_KEY_MEETING = "/meeting";
    public static final String REQUEST_MAPPING_KEY_TRANSACTION = "/transaction";
    public static final String REQUEST_MAPPING_KEY_FIND_ATTENDANCE_BY_MEETING_USER_ID = "/all-attendance-by-meeting/{id}";
    public static final String REQUEST_MAPPING_KEY_FIND_ALL_MEETING_BY_GROUP_ID = "/all-meeting-by-group/{id}";
    public static final String REQUEST_MAPPING_KEY_FIND_ALL_VISITORS_BY_MEETING = "/all-visitor-by-meeting/{id}";
    public static final String REQUEST_MAPPING_KEY_FIND_BY_GROUP_AND_ROLE = "/by-group-and-role/{id}/{roleName}";
    public static final String REQUEST_MAPPING_KEY_MAKE_CAPTAIN = "/make-captain";

    public static final String REQUEST_MAPPING_KEY_USER = "/user";
    public static final String REQUEST_MAPPING_KEY_ROLE = "/role";
    public static final String REQUEST_MAPPING_KEY_ROLE_BY_ROLE_NAME = "/role-name/{userRole}";
    public static final String REQUEST_MAPPING_KEY_AUTH = "/auth";
    public static final String REQUEST_MAPPING_KEY_USER_USER_ID = "/userId/{userId}";
    public static final String REQUEST_MAPPING_KEY_GROUP_GROUP_ID = "/groupId/{groupId}";

    public static final String REQUEST_MAPPING_KEY_USER_GROUP = "/user-group";
    public static final String REQUEST_MAPPING_KEY_GROUP = "/group";
    public static final String REQUEST_MAPPING_KEY_GROUP_SETTINGS = "/group-settings";
}
